...
*    @date        24/08/2019
*    @description Netcat replacement tool written in python. 
                  Inspired by Justin Seitz's book "Black Hat Python: Python Programming for Hackers and Pentesters"
...
def client_handler(client_socket):
    ...
    if len(upload_destination):
        ...
        # check for command execution and if it's ok we execute it
    if len(execute):
        output = run_command(execute)
        client_socket.send(output) # send the output
...
def server_loop():
    ...
    # if not target defined we listen on all interfaces
    if not len(target):
        target = '0.0.0.0'
...
# here we define the instructions of our script
def usage():

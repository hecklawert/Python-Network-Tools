...
# this is our server loop listening for incoming connections
...
def usage():
    print "Usage: ./pyproxy.py [localhost] [localport] [remotehost] [remoteport] [receive_first]"
    print "Example: ./pyproxy.py 127.0.0.1 9000 10.12.132.1 9000 True"

def main():
    
    # checking that we have all required arguments
    if len(sys.argv[1:]) != 5:
        usage()
        sys.exit(0)
    ...
    receive_first = sys.argv[5].lower()
    
    if "true" == receive_first:
        receive_first = True
    elif "false" == receive_first:
        receive_first = False
    else:
        usage()
        
    # now we launch our listening socket
